...
char	**cmd_paths(t_mini *mini)
{
	...
	if (!paths)
	{
		paths = ft_strdup("/Users/met-tahe/goinfre/homebrew/bin:/Users/met-tahe/goinfre/homebrew/bin:/Users/met-tahe/goinfre/homebrew/bin:/Users/met-tahe/goinfre/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/munki:/Library/Frameworks/Mono.framework/Versions/Current/Commands");
	}
...
void	heredoc_child(int *stat1, t_cmd *tmp1, char *line, int *j)
{
	...
	{
		(signal(SIGQUIT, ctrld_handler), signal(SIGINT, SIG_DFL));
		k = 1;
...
void	exec_cmd(t_mini *mini, t_cmd *cmd, char **env)
{
	...
	// // creer les pipes pour les heredoc;
	// pipe_herdoc(cmd);
	// ecrire dans les pipes de heredoc;
	...
	else
	{
		...
		while (!vars->stat1 && cmd && vars->i <= vars->pip_count)
		{
			...
			my_global->id[vars->i] = fork();
			//multiple_cmd(cmd, &i, pip_count, fd, paths, env, mini);
			// if (my_global->id[i] < 0)
			// 	fct_error("fork");
			// // child process;
			// else if (my_global->id[i] == 0)
			// {
			// 	//handler le signal sigint;
			// 	signal(SIGINT, SIG_DFL);
			// 	// mettre les resultats de la commande dans le pipe;
			// 	sort_in_pipe(pip_count, fd, i);
			// 	// executer les commandes builtins;
			// 	if (execute_builts(mini, cmd->cmd) == 1)
			// 		exit(my_global->status = 0);
			// 	child_execution(cmd, paths, env, i);
			// }
			// // incrementer a la commande suivante;
			// else
			// 	parent_waiting(&cmd, i, pip_count, fd);
